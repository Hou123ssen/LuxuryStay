...
export default function AdminPropertyRecentReports({ rows = [] }) {
  if (!rows.length) {
      ...
      {rows.map((report) => (
        <div key={report.id} className="rounded-xl border border-white/5 bg-black/20 p-3">
          <div className="flex items-start justify-between gap-3">
            <span className="text-sm font-medium text-cream">{report.reporter_name || `Reporter #${report.reporter_id}`}</span>
          ...
          <p className="mt-1 text-xs text-cream/30">{formatAdminPropertyDate(report.created_at)}</p>
        </div>
      ))}
    </div>
  );
